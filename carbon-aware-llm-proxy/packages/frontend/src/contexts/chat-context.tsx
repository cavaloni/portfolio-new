...
import { useSendMessage, useStreamMessage, useCreateConversation } from '@/hooks/use-chat';
...
export function ChatProvider({ children }: { children: ReactNode }) {
  ...
  const { models: availableModels, isLoading: isModelsLoading } = useCarbonAwareModels();
  ...
  }, [input, selectedModel, messages, currentConversationId, createConversation, sendMessage, isCarbonAware]);
  ...
  const handleCreateNewConversation = useCallback(async () => {
    try {
      ...
      setError(err instanceof Error ? err : new Error('Failed to create new conversation'));
      throw err;
    }
