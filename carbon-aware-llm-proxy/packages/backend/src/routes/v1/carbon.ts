...
const carbonEstimateSchema = z
  ...
  .refine(
    ...
    {
      message:
        "At least one of prompt_tokens, completion_tokens, or total_tokens must be provided",
    },
  );
...
carbonRouter.get("/intensity", (req, res) => {
  ...
  if (typeof region === "string") {
    const intensity =
      mockCarbonIntensity[region as keyof typeof mockCarbonIntensity];

    if (!intensity) {
      throw new ApiError(
        404,
        `No carbon intensity data available for region: ${region}`,
      );
    }
...
carbonRouter.post("/estimate", (req, res) => {
  ...
  const { model, region, prompt_tokens, completion_tokens, total_tokens } =
    validatedBody.data;
  ...
  const intensity =
    mockCarbonIntensity[effectiveRegion as keyof typeof mockCarbonIntensity];

  if (!intensity) {
    throw new ApiError(
      404,
      `No carbon intensity data available for region: ${effectiveRegion}`,
    );
  }
...
carbonRouter.post("/routing/preferences", (req, res) => {
  ...
  const totalWeight =
    preferences.carbon_weight + preferences.performance_weight;
