...
class ChatService {
  async createConversation(
    ...
    const conversationRepository = databaseService.getDataSource().getRepository(Conversation);
  ...
  async deleteConversation(userId: string, conversationId: string) {
    ...
    const result = await conversationRepository.delete({ id: conversationId, userId });
  ...
  async addMessage(
    ...
    const messageRepository = databaseService.getDataSource().getRepository(Message);
    const conversationRepository = databaseService.getDataSource().getRepository(Conversation);
    
    // Start a transaction if one isn't provided
    const shouldReleaseQueryRunner = !queryRunner;
    const qr = queryRunner || databaseService.getDataSource().createQueryRunner();
    ...
    try {
      ...
      if (conversation) {
        ...
        conversation.totalTokens = (conversation.totalTokens || 0) + (tokens || 0);
        
        if (carbonFootprint) {
          conversation.totalEmissions = (conversation.totalEmissions || 0) + carbonFootprint.emissions;
          conversation.totalEnergy = (conversation.totalEnergy || 0) + carbonFootprint.energy;
        }
  ...
  async deleteMessage(userId: string, messageId: string) {
    ...
    const result = await messageRepository
      ...
      .andWhere('conversationId IN (SELECT id FROM conversations WHERE userId = :userId)', { userId })
      .execute();
