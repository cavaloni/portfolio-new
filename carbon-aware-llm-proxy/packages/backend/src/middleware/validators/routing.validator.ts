...
export const getOptimalModelValidator = [
  ...
  (
    req: Request<{}, {}, UpdateWeightsBody, WeightsQuery>,
    res: Response,
    next: NextFunction,
  ) => {
    const { carbonWeight, performanceWeight, costWeight } = req.query;
    const weights = [carbonWeight, performanceWeight, costWeight].filter(
      (w) => w !== undefined,
    );
  ...
  (
    ...
    if (carbonWeight !== undefined) {
      const sum =
        parseFloat(carbonWeight) +
        parseFloat(performanceWeight || "0") +
        parseFloat(costWeight || "0");
...
export const updatePreferencesValidator = [
  ...
  (
    ...
    if (weights) {
      ...
      const providedWeights = [carbonEfficiency, performance, cost].filter(
        (w) => w !== undefined,
      );
