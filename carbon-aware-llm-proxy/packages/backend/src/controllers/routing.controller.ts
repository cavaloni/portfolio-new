...
export class RoutingController {
  ...
  async getOptimalModel(req: Request, res: Response) {
    try {
      ...
      const weights = {
        carbonEfficiency: carbonWeight ? parseFloat(carbonWeight as string) : undefined,
        performance: performanceWeight ? parseFloat(performanceWeight as string) : undefined,
      ...
      const result = await routingService.getOptimalModel({
        ...
        preferredProviders,
        weights: Object.values(weights).some(w => w !== undefined) ? weights : undefined,
      });
